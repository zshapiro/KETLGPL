		...
		synchronized (this.oLock) {
			...
			PreparedStatement mReadyList = this.metadataConnection
					.prepareStatement("SELECT load_id, job_id, status_id FROM " + tablePrefix
							+ "job_log AA WHERE STATUS_ID in (?, ?, ?) AND EXISTS " + " (SELECT 1 FROM  " + tablePrefix
							+ "job_log a,  " + tablePrefix + "job_dependencie b " + " WHERE a.job_id = b.job_id "
							+ " AND aa.load_id = a.load_id and aa.job_id = b.parent_job_id"
							+ " GROUP BY load_id,parent_job_id " + " HAVING MAX (CASE WHEN continue_if_failed = 'Y' "
