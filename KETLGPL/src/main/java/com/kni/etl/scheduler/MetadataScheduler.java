...
public class MetadataScheduler extends Metadata {
   ...
   */
  public ETLJob getNextJobInQueue(Collection<String[]> pJobTypes, int pServerID)
    ...
    synchronized (this.oLock) {
      ...
      getfinishedLoads =
          this.metadataConnection
              .prepareStatement("SELECT LOAD_ID FROM  "
                  + tablePrefix
                  + loadTableName()
      ...
      while (m_rs.next()) {
        ...
        if (setLoadEndDate == null) {
          setLoadEndDate =
              this.metadataConnection.prepareStatement(" UPDATE  " + tablePrefix + loadTableName()
                  + " SET END_DATE = " + currentTimeStampSyntax + " WHERE LOAD_ID = ?");
        }
        ...
        if (clearJobErrorStmt == null) {
          clearJobErrorStmt =
              this.metadataConnection.prepareStatement("delete from  " + tablePrefix
      ...
      while (m_rs.next() && scheduledJob == false) {
        ...
        if (updJobSched == null) {
          updJobSched =
              this.metadataConnection.prepareStatement(" UPDATE  " + tablePrefix
        ...
        if (nextRunDate == null || nextRunDate.equals(lastRunDate)) {
          ResourcePool.LogMessage(this, ResourcePool.INFO_MESSAGE, "Disabling schedule ["
              + scheduleID + "] next run date is null or the same as the previous date");
      ...
      PreparedStatement mReadyList =
          this.metadataConnection.prepareStatement("SELECT load_id, job_id, status_id FROM "
              + tablePrefix + "job_log AA WHERE STATUS_ID in (?, ?, ?) AND EXISTS "
      ...
      while (rsReadyJobs.next()) {
        ...
        switch (wait_status) {
          ...
          case ETLJobStatus.WAITING_TO_SKIP:
            ...
            updJobs.setString(2,
                etlJobStatus.getStatusMessageForCode(ETLJobStatus.PENDING_CLOSURE_SKIP));
            break;
      ...
      mRetryList.setString(4,
          etlJobStatus.getStatusMessageForCode(ETLJobStatus.PENDING_CLOSURE_FAILED));
      mRetryList.setInt(5, ETLJobStatus.WAITING_TO_BE_RETRIED);

      ResultSet m_rsJobsToRetry = mRetryList.executeQuery();

      PreparedStatement updRetryJobs =
          this.metadataConnection.prepareStatement(" UPDATE  " + tablePrefix + "job_log "
              + " SET status_id = ?, message = ?,retry_attempts = ? where job_id = ?");
      ...
      PreparedStatement selFinishedJobs =
          this.metadataConnection
              .prepareStatement(" SELECT STATUS_ID, DM_LOAD_ID, LOAD_ID, JOB_ID FROM  "
                  + tablePrefix + "JOB_LOG WHERE STATUS_ID IN (?,?,?, ?) FOR UPDATE");
      ...
      while (m_rs.next()) {
        ...
        // -- only if all parent jobs have been processed (are not in WAITING state)
          ...
          // -- for root jobs, test that all children have completed/not started (nothing is
          // running)
          case ETLJobStatus.PENDING_CLOSURE_CANCELLED:
            if (pCancelStmt == null) {
              pCancelStmt =
                  metadataConnection.prepareStatement("UPDATE  " + tablePrefix
                      + "JOB_LOG SET STATUS_ID = ?, MESSAGE = ? "
                      + " WHERE DM_LOAD_ID = ? AND (EXISTS (SELECT 1 FROM " + tablePrefix
                      + "LOAD WHERE LOAD_ID = ? AND START_JOB_ID <> ?) "
                      + " OR NOT EXISTS (SELECT 1 FROM  " + tablePrefix + "LOAD B, " + tablePrefix
                      + "JOB_LOG C " + " WHERE B.LOAD_ID = ? AND B.START_JOB_ID = ? "
            ...
            // we need to make sure a job in execution paused is stopped first/cleanly before
            // cancelling the
            // load
      ...
      if (ReturnNextJob == true) {
        ...
        for (String[] jobType : pJobTypes) {
          jobTypesSQL.append("(C.DESCRIPTION = '" + jobType[0] + "' AND nvl(B.POOL,'"
              + EngineConstants.DEFAULT_POOL + "') = '" + jobType[1] + "') OR ");
        }

        PreparedStatement getNextJob =
            metadataConnection
