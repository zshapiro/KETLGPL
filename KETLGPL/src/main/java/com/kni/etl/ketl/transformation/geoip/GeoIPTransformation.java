...
public class GeoIPTransformation extends ETLTransformation {
	...
	class GEOIPOutPort extends ETLOutPort {
		...
		public String generateCode(int portReferenceIndex) throws KETLThreadException {
			...
			return this.getCodeGenerationReferenceObject() + "[" + this.mesStep.getUsedPortIndex(this) + "] = ((" + this.mesStep.getClass().getCanonicalName()
					+ ")this.getOwner()).getValue(" + portReferenceIndex + ")";
		...
		final public void setDataTypeFromPort(ETLPort in) throws KETLThreadException, ClassNotFoundException {
			if (this.lookUpRef != -1) {
				...
				String declared = XMLHelper.getAttributeAsString(this.getXMLConfig().getAttributes(), "DATATYPE", required);

				if (required.equalsIgnoreCase(declared) == false)
					throw new KETLThreadException("Port " + this.mstrName + " requires datatype to be " + required, this);
				(this.getXMLConfig()).setAttribute("DATATYPE", required);
				this.setPortClass();
			} else
	...
	private static final String[] FIELDS = { "COUNTRYCODE", "COUNTRYNAME", "REGION", "CITY", "POSTALCODE", "LATITUDE", "LONGITUDE", "AREACODE", "DMA", "ISP", "ORG" };
	...
	private final boolean[] loadCache = { false, false, false, false };
	String mCurrentIP;
	...
	public GeoIPTransformation(Node pXMLConfig, int pPartitionID, int pPartition, ETLThreadManager pThreadManager) throws KETLThreadException {
	...
	protected String getRecordExecuteMethodHeader() throws KETLThreadException {
		...
		return super.getRecordExecuteMethodHeader() + " ((" + this.getClass().getCanonicalName() + ")this.getOwner()).loadValue(" + this.mSourceIPPort.generateReference() + ");";
	}
