...
public class KETLKernelImpl implements KETLKernel {
      ...
      for (int i = 0; i < numThreads; i++) {
        try {
          ResourcePool.LogMessage(
              ...
              "Thread #" + (i + 1) + " = " + listOfThreads[i].getName() + " of "
                  + Thread.activeCount());
        } catch (Exception e) {
        }
      }
  ...
  public void run(java.lang.String[] args) {
    ...
    if (serverXMLConfig != null) {
      ...
      if (mdServer == null) {
        ...
        String servername =
            XMLHelper.getAttributeAsString(n.getAttributes(), "DEFAULTSERVER", "localhost");
    ...
    try {
      ...
      if (serverExecutors == null) {
        ...
        for (int i = 0; i < serverExecutors.length; i++) {
          // get server executor
          jobManagers[i][0] =
              new ETLJobManager((String) serverExecutors[i][0],
                  ((Integer) serverExecutors[i][1]).intValue(),
                  ((Integer) serverExecutors[i][2]).intValue(), (String) serverExecutors[i][3],
                  (String) serverExecutors[i][4]);
          ...
          switch ((submittedJobsToCheck[pos]).getStatus().getStatusCode()) {
            case ETLJobStatus.PENDING_CLOSURE_FAILED:
              ...
              if ((submittedJobsToCheck[pos]).getMaxRetries() > (submittedJobsToCheck[pos])
                  .getRetryAttempts()) {
                (submittedJobsToCheck[pos]).getStatus().setStatusCode(
            ...
            case ETLJobStatus.EXECUTING:
              ...
              if (currentTime > (mdCheckStatusTimer + 1000)) {
                ...
                int xStatus =
                    md.getJobStatusByExecutionId((submittedJobsToCheck[pos]).getJobExecutionID());
                ...
                if (xStatus == ETLJobStatus.ATTEMPT_CANCEL) {
                  (submittedJobsToCheck[pos]).getStatus()
                      .setStatusCode(ETLJobStatus.ATTEMPT_CANCEL);
        ...
        if ((shutdown == false) && (paused == false)) {
          ...
          for (int pos = 0; pos < jobManagers.length; pos++) {
            if (((ETLJobManager) jobManagers[pos][0]).getStatus().getStatusCode() == ETLJobManagerStatus.READY) {
              jobTypesToRequest.add(new String[] {
                  ((ETLJobManager) jobManagers[pos][0]).getJobType(),
                  ((ETLJobManager) jobManagers[pos][0]).getPool()});
            }
          }
          ...
          if (job != null) {
            ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.INFO_MESSAGE,
                "Finding available executor for " + job.getJobID() + "[" + job.getLoadID() + "]");
  ...
  private Set<Integer> warningSent = new HashSet<Integer>();
  private long lastFailedServerCheckTime = 0;

  private void checkForFailedServers(MetadataScheduler md) {
    // check for failed servers every minute

    if (System.currentTimeMillis() > lastFailedServerCheckTime - (1000 * 60)) {
        ...
        for (Server s : md.getAliveServers()) {
          ...
          if (diff >= notifyTime) {
            if (warningSent.contains(s.mServerID) == false) {
              md.sendEmailToAll(
                  "KETL Server " + s.mName + " Offline",
                  s.mName
                      + " appears to be offline and has not pinged the metadata in "
                      + diff
                      + " seconds. All executing jobs assigned to this server will be marked for retry.");
              warningSent.add(s.mServerID);
              md.recoverServerJobs(s.mServerID);
            }
          } else {
            if (warningSent.remove(s.mServerID)) {
              md.sendEmailToAll("KETL Server " + s.mName + " Recovered", s.mName
                  + " appears to be online again and pinged the metadata in last " + diff
                  + " seconds.");
            }
          }
          lastFailedServerCheckTime = System.currentTimeMillis();
        }
      } catch (Exception e) {
  ...
  private void removeJobFromSubmittedJobs(Vector<ETLJob> submittedJobs, ETLJob job, Metadata md)
      throws SQLException, Exception {

    ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.INFO_MESSAGE,
        "Remove job from execution list " + job.getJobID() + ", final status - "
            + job.getStatus().getStatusMessage());
    ...
    if (finalStatus != ETLJobStatus.FATAL_STATE && initStatus != finalStatus) {
      ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.ERROR_MESSAGE,
          "Status issue, expected " + initStatus + " found " + finalStatus);
    }
  ...
  private boolean submitJob(Vector<ETLJob> submittedJobs, Object[][] jobManagers, ETLJob job) {
    ...
    for (int pos = 0; pos < jobManagers.length; pos++) {
      if (((ETLJobManager) jobManagers[pos][0]).getStatus().getStatusCode() == ETLJobManagerStatus.READY) {
        if (((ETLJobManager) jobManagers[pos][0]).submitJob(job) == true) {
          submittedJobs.add(job);
          ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.INFO_MESSAGE,
              "Submitting job " + job.getJobID() + " for execution, type = "
                  + ((ETLJobManager) jobManagers[pos][0]).getJobType() + ",  pool = "
                  + ((ETLJobManager) jobManagers[pos][0]).getPool());
          return true;
        } else {
          job.getStatus().setStatusCode(ETLJobStatus.READY_TO_RUN);
    ...
    ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.DEBUG_MESSAGE,
        "Job not submitted " + job.getJobID() + " no executors available, return to pending list");
    return false;
  }
